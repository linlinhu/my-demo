...
function QRCanvas(options) {
	...
	t.correctLevel=options.correctLevel||'H';
...
QRCanvas.prototype={
	...
	prepareImage:function() {
		...
		if(k=image.dom) {
			...
			k=w/h;
			nh=Math.floor(Math.sqrt(Math.min(w*h/t.size/t.size,.1)/k)*t.count*0.9);
			nw=Math.floor(k*nh*0.9);
			console.log(nh,nw)
	...
	drawImage:function() {
		...
		if(image.dom) {
			if(!image.clearEdges) {
				...
				console.log(image.x-image.margin,image.y-image.margin,image.width+2*image.margin,image.height+2*image.margin)
			}
			t.context.drawImage(image.dom,image.x,image.y,image.width,image.height);
			console.log(image.dom,image.x,image.y,image.width,image.height)		}
	},
	isDark:function(i,j) {
